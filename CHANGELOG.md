...
- **CI now checks a property of the whole adopter-visible surface, not the handful of methods a quickstart happens to call.** The publish workflow's adopter compile gate is necessary but not sufficient: it only exercises what its hand-written quickstart touches, which is precisely how v0.11.1's `page.swipe()` defect reached the registry — the quickstart calls `tap`/`type`/`snapshot`/`screenshot`/`findByText` and never calls `swipe`. Enumerating every method by hand does not scale and rots as methods are added. `scripts/check-adopter-types.mjs` (TypeScript compiler API, no new dependency) instead asserts two properties of the shipped `.d.ts`: no exported function returns a bare `object`, and the Android and iOS pages agree on the required-parameter count and declared return type of every method they both have. A method added tomorrow is covered without anyone remembering to extend a script. Wired into `ci.yml` (every push/PR) and `publish.yml` (before the tarball is packed); exits 1 on a defect and 2 if `types/` was never built, so a misconfigured step cannot look like a pass. Validated against real history rather than a fixture written to pass: it fires on `c006cac` (bare `object` on `connect()`/`translateWda()`, 3 findings), on `c4d60ce` (`page.swipe()`, 5 required vs 4), and on HEAD with the `screenshot()` fix reverted (`Promise<string>` vs `Promise<Buffer>`) — and reports zero on the shipped tree. CI only — no runtime or published-artifact change.
- **The same gate now checks every page member against its own source, not against the other platform's copy.** Comparing Android's copy of a method with iOS's can only find *disagreement*, which left two blind spots: a declaration wrong the same way on both platforms had nothing to contradict it, and the 13 of 32 page members that exist on one platform only (`intent`, `grantPermission`, `tapGrid`, … on Android; `unlock`, `lock`, `activate`, `baseUrl`, … on iOS) were never compared at all — 19 of 32 members carried any coverage. Two rules read `src/` instead of the built `.d.ts`, because the evidence lives in the function body a `.d.ts` drops: `forwarded-default` flags a parameter handed straight to another of our functions at a position that function defaults but restated without a default here, and `optional-in-fact` flags a required parameter whose every use sits on the left of `||`/`??`, where the body already handles its absence. Both judge a member against its own implementation, so they give a notion of *wrong* rather than only *these two disagree*, and single-platform members are covered. Validated on real defects, not fixtures: `forwarded-default` fires on `c4d60ce`'s `page.swipe()` from source alone; with that same default removed on *both* pages the original two rules report "OK" while `forwarded-default` still fires — the first blind spot, demonstrated; and `optional-in-fact` fired on shipped `HEAD` against iOS-only `page.unlock()`, a live defect no parity check could see (fixed below). Parameters are resolved by symbol rather than name text, so a nested closure shadowing a parameter cannot misattribute a finding, and shorthand property uses (`post({ pin })`) count as real uses; an argument after a spread is skipped, since its callee position is unknowable. Residual gaps are stated in the script header rather than left implicit: this is absolute for parameter *optionality*, not for return types, and `optional-in-fact` would misread a fallback that itself throws. CI only — no runtime or published-artifact change.

### Fixed

- **`page.unlock()` on iOS forced adopters to pass a passcode `connect()` already held.** The wrapper was written `async unlock(pin)` with no default, so the generated declaration was `unlock(pin: any)` — required — even though the body reads `pin || passcode` and falls back to the passcode given to `connect({ passcode })`. A TypeScript adopter calling the idiomatic `page.unlock()` therefore got `TS2554: Expected 1 arguments, but got 0`, and `any` gave them no type either. Now declared `unlock(pin?: string | null)` via JSDoc. Found by the new `optional-in-fact` rule on the shipped tree, not by hand — and invisible to the parity rule, since `unlock` is iOS-only and has no Android counterpart to disagree with. Declaration-only: the change is a JSDoc comment, no runtime code, verified against a packed tarball — a consumer calling `page.unlock()` gets `TS2554` before and compiles clean after. 321 tests unchanged and green.
...
- **Publish workflow gates on the types being usable BY AN ADOPTER, not just internally.** `npm run typecheck` (`tsc --noEmit`) checks the *source*; it cannot see the generated `.d.ts` as an adopter resolves it from inside `node_modules`, which is the one thing consumers actually get. The publish workflow now packs the tarball, installs it into a clean consumer project, and compiles a quickstart against it, so a release whose published types are broken cannot reach the registry. The quickstart *dereferences* what it imports — a name that only resolves proves nothing about its shape — so it reads fields off the page objects and the translated node tree, not just off `snapshot()`'s string.  The consumer pins `@types/node` to the major this package builds against instead of floating to the newest, so a stricter DefinitelyTyped release cannot turn the publish gate red for reasons unrelated to the commit being published. Verified locally: the quickstart compiles green against a packed tarball, and a deliberately broken dereference fails it. CI only — no runtime or published-artifact change.

### Fixed

- **`connect()` and `translateWda()` no longer return a bare `object` to TypeScript adopters.** Both were annotated `@returns {object}`, and a bare `object` accepts no property access — so an adopter who did `const page = await connect()` could not read `page.serial` or call `page.tap()` without casting, and the same held for `translateWda()`'s node tree. The two `@returns` are now the real shapes: `connect()` (both platforms) lets `tsc` infer the page structurally from the literal it returns, exported under the new `AndroidPage` / `IosPage` type aliases; `translateWda()` returns the already-shipped `UiNode | null`. Also corrected `page.screenshot()` on Android, which the inferred type exposed as declaring `Promise<string>` while returning PNG bytes — now `Promise<Buffer>`, matching iOS. Verified by packing the tarball into a clean consumer project and compiling a quickstart that dereferences all of it: green after, and four errors before. Types only — no runtime change, 321 tests unchanged and green.
